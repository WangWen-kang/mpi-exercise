...
! Every process copies its v_lcl to its place in v_glb:
      do i = 0 , counts(myid)-1
         v_glb(dspls(myid)+i) = v_lcl(i)
      end do
!Every process broadcasts its part of v_glb
      do ip = 0 , np-1
         call MPI_BCAST(v_glb(dspls(ip)), counts(ip),
     :                  MPI_DOUBLE_PRECISION,ip,MPI_COMM_WORLD, ierr)
      end do
