...
/*
  ...
  |                                                                   |
  |   Distributed version 1 - column blocks                           |
  +-------------------------------------------------------------------+
*/
...
{ 
   ...
   /*  Initialization on proc 0 -> only root has the whole matrix */
   if (myid == 0){
...
/*      printf("myid: %d \n",myid);
   ...
   /* Calculate index for each proc */
   dist_index( n, firstind );
   nl = firstind[myid+1] - firstind[myid];

   /* Distribute matrix among procs */
   dist_matrix_colblock( n, n, a, al );
   MPI_Barrier( MPI_COMM_WORLD );
/*      for (i = 1; i <= n;i++){
         printf("%d : %f %f %f %f \n",myid, al[i-1],al[i-1+n],al[i-1+2*n],al[i-1+3*n]);
         }*/
...
*/
   ...
   for (il = 1; il <= nloop;il++){
      ...
      t2 = MPI_Wtime();
      /* Make sure all are finished */
      MPI_Barrier( MPI_COMM_WORLD );
      /* Calculate final result, return in 'x' */
      reduce_vector(n,y,x);

      t3 = MPI_Wtime();

      if (myid == 0){
         /* Get eigenvector and calculate difference to last iteration */
         evn = x[0];
         res = (evn - evo) / evn;
//         printf("%d %20.15f %e \n",il,evn,res);
         /* Set flag if we're finished */
         if (fabs(res) < 1.0e-12) istop = 1;
      }

      /* Broadcast result and flag*/
      MPI_Bcast( &evn, 1, MPI_DOUBLE, 0, MPI_COMM_WORLD );
      MPI_Bcast( &istop, 1, MPI_INT, 0, MPI_COMM_WORLD );
      /* Save this value for next iteration */
      evo = evn;
